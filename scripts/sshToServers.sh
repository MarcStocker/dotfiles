...
exit_status()
{
  case $exitStatus in
    0 | 1 | 130 | 127 )
      ...
      echo -e "${prefix}${LIGHTGREEN}Connection Closed Gracefully${NOCOLOR}"
      exit_script
      ;;
  esac
  ...
  if [[ $reconnectCount -gt 6 ]]; then
    return
  fi
  if [[ (( $errorCode -eq 22 )) || (( $errorCode -eq 42 )) ]]; then
    if [[ ${reconnectCount} -gt 5 ]]; then
      return 0
    elif [[ ${RECONNECT^^} == 'Y' || ${RECONNECT^^} == "YES" ]]; then
      echo "Reconnect Count: $reconnectCount"
      ((reconnectCount++))
      connect
      exit_status
    fi
      echo -e "${prefix}${PURPLE}Would you like to continue trying to reconnect?${NOCOLOR}"
      echo -en "${prefix}[Y/N]: ${NOCOLOR}"
      read RECONNECT
      echo -en "${NOCOLOR}"
      case ${RECONNECT^^} in 
        Y | YES)
